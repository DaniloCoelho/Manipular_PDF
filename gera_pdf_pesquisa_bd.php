<?php 
    ...
    if(isset($_GET['texto'])){
        $text = "%".$_GET['texto']."%";
        $sql = "SELECT id_usuarios ,nome ,email ,data_nasc FROM usuarios WHERE nome  LIKE :texto OR email LIKE :texto ";
        $query = $con->prepare($sql);
        $query->bindParam(':texto' ,$text , PDO::PARAM_STR);
        //$query->bindParam(':texto_b' ,$text , PDO::PARAM_STR);
        $query->execute();
    }
